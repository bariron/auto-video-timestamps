...
from video_timestamps.sources import yandex_disk
...
class YandexDiskTests(unittest.TestCase):
    def test_public_urls(self):
        for host in ["disk.yandex.ru", "disk.yandex.com", "yadi.sk"]:
            self.assertEqual(
                yandex_disk.normalize_public_url(f"http://{host}/d/abc-123/?x=1"),
                f"https://{host}/d/abc-123",
            )
        for url in [
            "https://disk.yandex.ru.evil.org/i/abc",
            "file:///i/abc",
            "https://user@disk.yandex.ru/i/abc",
    ...
    def test_api_download_and_encoded_folder_path(self):
        responses = [
            io.BytesIO(json.dumps({"type": "file", "name": "../../video.mp4", "size": 5}).encode()),
            io.BytesIO(b'{"href":"https://download.example/media", "method":"GET"}'),
            io.BytesIO(b"video"),
        ]
        with (
            tempfile.TemporaryDirectory() as directory,
            patch.object(yandex_disk, "urlopen", side_effect=responses) as request,
        ):
            result = yandex_disk.download_public_media(
                "https://disk.yandex.ru/d/key", Path(directory), "/Лекция 1.mp4"
            )
    ...
    def test_http_error_is_readable(self):
        with patch.object(
            yandex_disk, "urlopen", side_effect=HTTPError("url", 404, "missing", {}, None)
        ):
    ...
    def test_cli_routes_disk_and_cleans_temporary_file(self):
        ...
        with (
            ...
            patch.object(pipeline, "download_youtube_audio") as youtube,
