...
from video_timestamps.jobs import JobQueue, validate_submission
...
class WebAppTests(unittest.TestCase):
    ...
    def test_nested_disk_link_and_validation(self):
        ...
        for payload in [
            ...
            {"url": "file:///secret"},
            {"url": "https://youtu.be/abcdefghijk", "disk_path": "/invalid"},
            {"url": "https://disk.yandex.ru/d/key/a.mp4", "disk_path": "/b.mp4"},
        ]:
            with self.subTest(payload=payload):
                self.assertEqual(
                    self.client.post(
                        "/api/jobs", json=payload, content_type="application/json"
                    ).status_code,
                    400,
                )
    ...
    def test_request_boundaries(self):
        ...
        self.assertEqual(
            self.client.post(
                "/api/jobs", json={}, headers={"Origin": "https://evil.example"}
            ).status_code,
            403,
        )
    ...
    def test_persistent_queue_and_interrupted_jobs(self):
        ...
        self.assertEqual(
            JobQueue(self.output, start_worker=False).list_jobs()[0]["status"], "queued"
        )
    ...
    def test_worker_success_and_failure_preserve_transcript(self):
        for code in [0, 1]:
            with self.subTest(code=code):
                job = self.queue.submit({"url": "https://youtu.be/abcdefghijk"})

                def launch(command, **kwargs):
                    directory = Path(command[command.index("--output-dir") + 1])
                    self.assertNotIn("--max-duration-seconds", command)
                    self.assertNotIn("--skip-chapters", command)
                    (directory / "video_result.json").write_text('{"chunks": []}', encoding="utf-8")
                    (directory / "video_timestamps.txt").write_text(
                        "00:00:00 Речь", encoding="utf-8"
                    )
                    if code == 0:
                        (directory / "video_chapters.txt").write_text(
                            "00:00:00 Введение", encoding="utf-8"
                        )
                ...
                with patch("video_timestamps.jobs.subprocess.Popen", side_effect=launch):
                ...
                self.assertIn(
                    "Processing chunk",
                    self.client.get("/api/jobs/" + job["id"] + "/log").json["text"],
                )

    def test_existing_artifacts_and_download_allowlist(self):
        (self.output / "lecture_result.json").write_text('{"chunks": []}')
        (self.output / "lecture_chapters.txt").write_text(
            "00:00:00 <script>text</script>", encoding="utf-8"
        )
        result = self.client.get("/api/results").json[0]
        self.assertIn("chapters", result["files"])
        self.assertIn("updated", result)
        self.assertEqual(
            self.client.get("/api/results/" + result["id"]).json["chapters"],
            "00:00:00 <script>text</script>",
        )
        ...
        response.close()
        self.assertEqual(
            self.client.get("/api/results/" + result["id"] + "/download/job.json").status_code, 404
        )
