...
from video_timestamps.chapters import (
    build_detail_batches,
    generate_detailed,
    generation_signature,
    validate_detail_output,
)
...
class DetailedChapterTests(unittest.TestCase):
    ...
    def test_chapters_cover_all_batches_without_global_ten_chapter_limit(self):
        batches = build_detail_batches(
            [{"start": i * 60, "text": f"Topic {i}"} for i in range(160)]
        )
        seen = []
                ...
                [
                    {
                        ...
                        "title": "Specific mathematical proof technique",
                    }
                    for s in (batch[0], batch[-1])
                ]
            )
    ...
    def test_rejects_fabricated_ids_generic_titles_and_missing_tail(self):
        ...
        for items in [
            ...
            [
                {"segment_id": 0, "title": "A concrete first topic"},
                {"segment_id": 1, "title": "A concrete second topic"},
            ],
            [
                {"segment_id": 0, "title": "A concrete first topic"},
                {"segment_id": 0, "title": "A duplicated first topic"},
            ],
        ]:
    ...
    def test_reuses_only_valid_checkpoint_batches(self):
        ...
        self.assertNotEqual(
            first, generation_signature([{"text": "original"}], "different-model", [batch])
        )
